...
/// </remarks>
public class EntityReaderFromCsv : IEntityReader
{
    ...
    public List<Institution> Read()
    {
        ...
        var fileMetaData = _csvExplorer.GetCsvFilePath();

        using var reader = new StreamReader(fileMetaData.filePath);
        using var csv = new CsvReader(reader, _configuration);
        csv.Context.RegisterClassMap<InstitutionMapper>();

        var institutions = csv.GetRecords<Institution>();
        if (fileMetaData.type is not InstitutionType.Unknown)
        {
            return institutions.Select(i =>
            {
                i.InstitutionType = fileMetaData.type;
                return i;
            }).ToList();
        }
